...
class TaskEngine:
    ...
    def __init__(self, config: Config):
        ...
        self.terraform_cli = TerraformCLI(
            terraform_path=config.terraform_path, working_dir=config.terraform_dir
        )
    ...
    def get_project_data(self, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        if (
            force_refresh
            or self._project_cache is None
    ...
    def create_task(self, query: str) -> Task:
        ...
        task = Task(
            id=task_id, query=query, status=TaskStatus.PENDING, created_at=time.time()
        )
    ...
    async def _execute_simple_query(self, task: Task) -> Dict[str, Any]:
        ...
        return {"action": "query", "query": task.query, "project_data": project_data}
